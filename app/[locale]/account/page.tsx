...
export default function MyAccountPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default: return "bg-dark-400 text-gray-200"
    }
  }
    ...
    <div className="min-h-screen bg-dark-500">
      ...
      <section className="pt-32 pb-16">
        <div className="container mx-auto px-4 sm:px-6">
          ...
          >
            <Link href="/" className="inline-flex items-center text-gray-400 hover:text-white transition-colors mb-4 sm:mb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                ...
                <Link href="/products" className="order-1 sm:order-2">
                  <Button className="bg-black text-white hover:bg-dark-700 w-full sm:w-auto">
                ...
                <CardHeader className="pb-2 sm:pb-4">
                  ...
                  <div className="flex border-b mt-3 sm:mt-4 -mx-1" role="tablist" aria-label="Account activity tabs">
                    <button
                      ...
                      className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 sm:py-3 font-medium text-center transition-colors focus:outline-none focus:ring-2 focus:ring-gold-500 focus:ring-inset ${
                        activeTab === 'orders' 
                            ...
                            <div key={order.id} className="border rounded-lg p-3 sm:p-4 hover:shadow-md transition-shadow">
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3 sm:mb-4">
                                <div className="min-w-0 flex-1">
                                  ...
                                  <p className="text-xs sm:text-sm text-gray-400">{new Date(order.createdAt).toLocaleDateString()}</p>
                                </div>
                                  ...
                                  <div key={index} className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                                    ...
                                    <div className="flex items-center justify-between sm:justify-end gap-3 sm:flex-shrink-0">
                                      ...
                                      {order.status === "delivered" && (
                                        item.review ? (
                                          <div className="flex items-center text-xs text-gray-400 bg-dark-400 px-3 py-2 rounded border border-dark-400 transition-all duration-200">
                      ...
                      <div className="space-y-4">
                        {userReviews.length === 0 ? (
                          <div className="text-center py-8">
                            ...
                            <p className="text-sm text-gray-400">Your product reviews will appear here</p>
                          </div>
                        ) : (
      ...
      {reviewModalOpen && currentOrder && currentItem && (
        ...
        >
          <motion.div
            ...
            className="bg-dark-600 rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto"
          >
                ...
                <div className="flex justify-center space-x-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      className="text-2xl focus:outline-none p-2 rounded-full hover:bg-dark-400 transition-colors"
                    >
                      <Star
      ...
      {orderDetailsOpen && selectedOrder && (
        ...
        >
          <motion.div
            initial={{ scale: 0.95, opacity: 0, y: 100 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            className="bg-dark-600 rounded-t-lg sm:rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto"
          >
            <div className="p-4 sm:p-6">
              {/* Mobile handle bar */}
              <div className="w-12 h-1 bg-gray-300 rounded-full mx-auto mb-4 sm:hidden"></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="text-gray-400">Name:</span> {selectedOrder.shippingAddress?.name || 'N/A'}</p>
                    <p><span className="text-gray-400">Phone:</span> {selectedOrder.shippingAddress?.phone || 'N/A'}</p>
                    <p><span className="text-gray-400">Address:</span> {selectedOrder.shippingAddress?.street || 'N/A'}</p>
                    <p><span className="text-gray-400">City:</span> {selectedOrder.shippingAddress?.city || 'N/A'}</p>
                    <p><span className="text-gray-400">Governorate:</span> {selectedOrder.shippingAddress?.governorate || 'N/A'}</p>
                    <p><span className="text-gray-400">Postal Code:</span> {selectedOrder.shippingAddress?.postalCode || 'N/A'}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="text-gray-400">Order Date:</span> {new Date(selectedOrder.createdAt).toLocaleString()}</p>
                    <p><span className="text-gray-400">Status:</span> 
                      <Badge className={`ml-2 ${getStatusColor(selectedOrder.status)}`}>
                        {getStatusText(selectedOrder.status)}
                      </Badge>
                    </p>
                    <p><span className="text-gray-400">Payment Method:</span> {selectedOrder.paymentMethod || 'N/A'}</p>
                    <p><span className="text-gray-400">Shipping Method:</span> Standard Delivery</p>
                  </div>
                </div>
              </div>

              <div className="border rounded-lg">
                <div className="p-4 bg-dark-500 border-b">
                    ...
                    <div key={index} className="p-4 flex items-start">
                      ...
                      <div className="flex-1">
                        ...
                        {item.isGiftPackage && item.packageDetails && (
                          <div className="mt-2 p-2 bg-dark-500 rounded border">
                            <div className="flex items-center space-x-2 mb-1">
                              <Package className="h-3 w-3 text-gray-400" />
                              <span className="text-xs font-medium text-gray-300">Package Contents:</span>
                            </div>
                            <div className="space-y-1">
                              {item.packageDetails.sizes.map((sizeInfo: any, sizeIndex: number) => (
                                <div key={sizeIndex} className="flex items-center space-x-2 text-xs">
                                  ...
                                  <span className="text-gray-400">{sizeInfo.size} ({sizeInfo.volume}):</span>
                                  <span className="font-medium text-gray-200">{sizeInfo.selectedProduct.productName}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm text-gray-400">{formatPrice(item.price || 0)} each</p>
                      </div>
                    </div>
                  ))}
                </div>
