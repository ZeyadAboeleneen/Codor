...
export function VariantsTab({ models, setModels, products, brands, getAuthToken }: ModelsTabProps) {
  ...
  const handleMultipleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(error.message || t("someImagesFailedUpload"))
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.name_en || !form.product_id || !form.brand_id || !form.price) { toast.error(t("requiredFieldsVariant")); return }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setModels(models.filter(m => m.id !== id)); toast.success(t("modelDeleted")) }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h2 className="text-xl font-semibold text-white flex items-center gap-2">
          <Cpu className="h-5 w-5 text-gold-400" /> {t("modelsTitle")} ({filtered.length})
        </h2>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <Select value={filterBrand} onValueChange={v => { setFilterBrand(v); setFilterProduct("all") }}>
            <SelectTrigger className="bg-dark-500 border-white/10 text-white w-[140px]"><SelectValue placeholder={t("allBrandsFilter")} /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={filterProduct} onValueChange={setFilterProduct}>
            <SelectTrigger className="bg-dark-500 border-white/10 text-white w-[140px]"><SelectValue placeholder={t("allProductsFilter")} /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("allProductsFilter")}</SelectItem>
              {filterProductsForList.map(p => <SelectItem key={p.id} value={p.id}>{p.name_en}</SelectItem>)}
            </SelectContent>
          </Select>
          <Button onClick={() => { resetForm(); setShowForm(true) }} className="bg-gold-500 text-dark-900 hover:bg-gold-400 whitespace-nowrap" size="sm">
            <Plus className="h-4 w-4 ml-2" /> {t("addModelButton")}
          </Button>
        </div>
      </div>
        ...
        <Card className="bg-dark-400 border-white/10">
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg text-white">{editing ? t("editModel") : t("addNewModel")}</CardTitle>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="text-gold-400 border-b border-white/10 pb-2 text-sm font-semibold">{t("basicInfo")}</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select value={form.brand_id} onValueChange={v => setForm({...form, brand_id: v, product_id: ""})}>
                      ...
                      <SelectContent>{brands.map(b => <SelectItem key={b.id} value={b.id}>{b.name_en}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Label className="text-gray-300 text-sm">{t("productTypeSelect")}</Label>
                    <Select value={form.product_id} onValueChange={v => setForm({...form, product_id: v})}>
                      <SelectTrigger className="mt-1 bg-dark-500 border-white/10 text-white"><SelectValue placeholder={t("productTypeSelect")} /></SelectTrigger>
                      <SelectContent>
                        {filteredProductsForBrand.length === 0
                          ? <SelectItem value="" disabled>{t("selectBrandFirst")}</SelectItem>
                          : filteredProductsForBrand.map(p => <SelectItem key={p.id} value={p.id}>{p.name_en}</SelectItem>)
                        }
                      </SelectContent>
                    </Select>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <Label className="text-gray-300 text-sm">{t("modelNameEn")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("modelNameAr")}</Label>
              ...
              {/* 2. Pricing & Stock */}
              <div className="space-y-4">
                <h3 className="text-gold-400 border-b border-white/10 pb-2 text-sm font-semibold">{t("priceAndStock")}</h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <Label className="text-gray-300 text-sm">{t("priceEGP")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("priceBeforeSale")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("stockQuantity")}</Label>
              ...
              {/* 3. Specs */}
              <div className="space-y-4">
                <h3 className="text-gold-400 border-b border-white/10 pb-2 text-sm font-semibold">{t("technicalSpecs")}</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div>
                    <Label className="text-gray-300 text-sm">{t("hpLabel")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("kwLabel")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("maxHeadLabel")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("maxFlowLabel")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("voltageFieldLabel")}</Label>
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("phaseLabel")}</Label>
                ...
                <div className="pt-2">
                  <Label className="text-gray-300 text-sm mb-2 block">{t("additionalSpecs")}</Label>
                  <div className="flex gap-2 mb-2">
                    <Input value={specKey} onChange={e => setSpecKey(e.target.value)} placeholder={t("specNamePlaceholder")} className="bg-dark-500 border-white/10 text-white flex-1" />
                    <Input value={specVal} onChange={e => setSpecVal(e.target.value)} placeholder={t("specValuePlaceholder")} className="bg-dark-500 border-white/10 text-white flex-1" />
              ...
              {/* 4. Media */}
              <div className="space-y-4">
                <h3 className="text-gold-400 border-b border-white/10 pb-2 text-sm font-semibold">{t("mediaAndDescription")}</h3>

                <div className="space-y-3">
                  <Label className="text-gray-300 text-sm">{t("modelImages")}</Label>
                  <div className="p-4 border-2 border-dashed border-white/10 rounded-xl bg-dark-500/50">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      >
                        ...
                        {isUploading ? `${t("selectImages")} ${uploadProgress.current}/${uploadProgress.total}` : t("selectImages")}
                      </Button>
                      <input type="file" ref={fileInputRef} onChange={handleMultipleFileUpload} accept="image/*" multiple className="hidden" />
                    </div>
                    {imagesList.length > 0 && (
                        ...
                        {imagesList.map((url, i) => (
                          <div key={i} className="relative w-24 h-24 rounded-lg overflow-hidden border border-white/20 group">
                            ...
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <button type="button" onClick={() => removeImage(i)} className="bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition-colors">
                                <X className="w-4 h-4" />
                              </button>
                            </div>
                          </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label className="text-gray-300 text-sm">{t("descriptionEn")}</Label>
              ...
              <label className="flex items-center gap-2 cursor-pointer bg-white/5 p-3 rounded-lg w-max border border-white/10 hover:border-gold-500/30">
                ...
                <span className="text-white text-sm font-medium">{t("featuredModel")}</span>
              </label>

              <Button type="submit" className="w-full bg-gold-500 text-dark-900 hover:bg-gold-400 font-semibold h-12" disabled={isUploading}>
                <Save className="h-5 w-5 ml-2" /> {editing ? t("saveChanges") : t("addModelButton")}
              </Button>
            </form>
          </CardContent>
        </Card>
      ...
      {filtered.length === 0 ? (
        <Card className="bg-dark-400 border-white/10">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-400">{products.length === 0 ? t("addBrandFirst") : t("noModelsYet")}</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-white text-lg truncate">{v.name_en}</p>
                      <p className="text-xs text-gray-400 mt-0.5">
                        <span className="text-gold-400">{v.brand_name_en || v.brand_id}</span> → <span className="text-blue-400">{v.product_name_en || v.product_id}</span>
                        {v.hp && <span className="text-gray-300"> • <Badge variant="outline" className="text-[10px] h-4 border-white/10">{v.hp}</Badge></span>}
                      </p>
                      <div className="flex items-center gap-2 mt-2">
                        <span className="text-gold-400 font-bold">{v.price.toLocaleString()} EGP</span>
                        {v.before_sale_price && v.before_sale_price > v.price && (
                          <span className="text-gray-500 text-xs line-through">{v.before_sale_price.toLocaleString()} EGP</span>
                        )}
                        {v.is_featured && <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30 text-[10px] h-5">{t("featuredBadge")}</Badge>}
                        {v.images?.length > 1 && <Badge variant="outline" className="text-[10px] h-5 bg-white/5 border-white/10">+{v.images.length - 1}</Badge>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    <Badge className={`cursor-pointer text-xs ${v.is_active ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-gray-500/20 text-gray-400 border-gray-500/30"}`} onClick={() => toggleActive(v)}>
                      {v.is_active ? t("activeStatus") : t("inactiveStatus")}
                    </Badge>
                    {v.stock === 0 && <Badge className="bg-red-500/20 text-red-400 border-red-500/30 text-xs">{t("outOfStockStatus")}</Badge>}
