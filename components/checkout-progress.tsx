  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <div className="w-full h-3 bg-dark-300 rounded-full" />
        
        {/* Filled Progress Bar */}
        <motion.div 
          className="absolute top-0 left-0 h-3 bg-gold-gradient rounded-full"
          initial={{ width: 0 }}
          ...
          {steps.map((step, index) => (
            <motion.div 
              ...
              className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                step.id <= currentStep 
                  ? 'bg-gold-gradient text-dark-900 border-2 border-white shadow-lg' 
                  : 'bg-gray-300 text-gray-400 border-2 border-white'
              }`}
            >
              {step.id}
            </motion.div>
